...
class TeslaMCP:
    ...
    async def get_system_summary(self) -> Dict[str, Any]:
        ...
        try:
            vehicles_response = await self.get_vehicles() # This returns the list directly now
            solar_systems_response = await self.get_solar_systems() # This returns the list directly

            # Assuming the structure of vehicle and solar objects from the original summary method
            # This might need adjustment based on actual API response structure
            return {
                "timestamp": datetime.now().isoformat(),
                "vehicles": [
                    {
                        "id": v.get("id_s", v.get("id")), # vehicle ID might be 'id_s' or 'id'
                    ...
                    for v in vehicles_response # vehicles_response is now the list
                ],
                "solar_systems": [
                    {
                        "id": s.get("energy_site_id", s.get("id")), # site ID field name might vary
                        "name": s.get("site_name"),
                        "status": s.get("status"), # This field might not exist, or be different (e.g., 'sync_status' for powerwall)
                        "total_power": s.get("total_pack_energy"), # Or another field like 'solar_power' or 'grid_power' from live_status
                        "battery_level": s.get("percentage_charged") # Or 'backup_reserve_percent'
                    }
                    for s in solar_systems_response # solar_systems_response is now the list
                ]
            }
        except Exception as e:
            return {
